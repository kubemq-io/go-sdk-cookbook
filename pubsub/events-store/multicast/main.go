...
func main() {
	...
	receiverA, err := kubemq.NewClient(ctx,
		kubemq.WithAddress("localhost", 50000),
		kubemq.WithClientId("go-sdk-cookbook-pubsub-events-store-multicast-receiver-a"),
		kubemq.WithTransportType(kubemq.TransportTypeGRPC),
	...
	receiverB, err := kubemq.NewClient(ctx,
		kubemq.WithAddress("localhost", 50000),
		kubemq.WithClientId("go-sdk-cookbook-pubsub-events-store-multicast-receiver-b"),
		kubemq.WithTransportType(kubemq.TransportTypeGRPC),
		kubemq.WithAutoReconnect(true))
	if err != nil {
		log.Fatal(err)
	...
	multicastChannel := "events-store.a;events-store.b"
	go func() {
		errCh := make(chan error)
		eventsCh, err := receiverA.SubscribeToEventsStore(ctx, "events-store.a", "", errCh, kubemq.StartFromFirstEvent())
		if err != nil {
			log.Fatal(err)
			return
	...
	go func() {
		errCh := make(chan error)
		eventsCh, err := receiverB.SubscribeToEventsStore(ctx, "events-store.b", "", errCh, kubemq.StartFromFirstEvent())
		if err != nil {
			log.Fatal(err)
			return
		}
